...
<style global>
	* { box-sizing: border-box; margin: 0; padding: 0; }
	...
	/* --- INFO TABLES --- */
	.info-section { padding: 3rem 2rem; max-width: 1200px; margin: 0 auto; }
	.info-container {
	...
	.day, .label { font-size: 1rem; color: #fff; font-weight: 500; }
	.time, .value { font-size: 1rem; color: #f68b1f; font-weight: 600; text-decoration: none; }
	.value:hover { text-decoration: underline; }

	/* --- CONTACT FORM --- */
	.contact { padding: 5rem 2rem; background: #1f1f1f; }
	.contact-inner { max-width: 700px; margin: 0 auto; text-align: center; }
	.contact-form {
		margin-top: 3rem; display: flex; flex-direction: column; gap: 1.2rem; text-align: left;
	}
	.contact-form input, .contact-form textarea {
		padding: 1rem 1.2rem; border-radius: 10px; border: 2px solid transparent;
		background: #2a2a2a; color: #fff; font-size: 1rem; font-family: inherit;
		transition: border-color 0.2s ease;
	}
	.contact-form input::placeholder, .contact-form textarea::placeholder { color: #888; }
	.contact-form textarea { resize: vertical; min-height: 140px; }
	.contact-form input:focus, .contact-form textarea:focus { outline: none; border-color: #f68b1f; }
	.contact-form .btn-primary { align-self: center; border: none; cursor: pointer; font-family: inherit; }
	...
	.btn-primary:hover { transform: translateY(-2px); box-shadow: 0 8px 20px rgba(246,139,31,0.4); }
	...
	.modal {
		background: #2a2a2a; border-radius: 12px; padding: 2rem 2.5rem;
		max-width: 420px; width: 90%; text-align: center; border-top: 4px solid #f68b1f;
	}
	.modal.modal-error { border-top-color: #e05252; }
	.modal h2 { margin-bottom: 1rem; font-size: 1.4rem; }
	.modal p  { margin-bottom: 1.5rem; color: #e0e0e0; }
	...
	@media (max-width: 767px) {
		...
		.subtitle { font-size: 1.15rem; margin-top: 0.65rem; }
		.hero     { padding: 2rem 1.25rem; }

		.info-section   { padding: 2rem 1.25rem; }
		.info-container { grid-template-columns: 1fr; gap: 1.25rem; }

		/* Email wraps gracefully */
		.table-row {
			flex-wrap: wrap; gap: 0.25rem; padding: 1rem 1.25rem;
		}
		.value { word-break: break-all; }

		.contact      { padding: 3rem 1.25rem; }
		.contact-form { gap: 1rem; margin-top: 2rem; }
		.contact-form input, .contact-form textarea { font-size: 1rem; padding: 0.9rem 1rem; }

		/* Full-width submit button */
		.contact-form .btn-primary {
			align-self: stretch; width: 100%; text-align: center; font-size: 1rem;
		}
	...
	@media (max-width: 479px) {
		...
		.day, .label, .time, .value { font-size: 0.9375rem; }
