...
export const load: PageServerLoad = async ({ cookies }) => {
	...
	const bookings = db.prepare(
		...
		id: number; customer_type: string; service: string; name: string;
		email: string | null; phone: string | null; dog_name: string;
		date: string; time: string; status: 'pending' | 'approved' | 'declined';
	...
	return { bookings, weekly, blocked, contacts };
};